...
+ [调整] 全站移除应用版本号展示及版本更新弹窗。
+ [调整] 主导航隐藏生图与视频工作台入口，主界面移除 Agent、文档及全站 GitHub 图标。
+ [修复] 画布远端视频任务刷新后自动恢复轮询，查询并下载会严格使用原模型渠道并如实保留上游进度。
+ [调整] 933 即梦不再在本地限制普通、负面及分镜提示词长度，超长内容改由上游服务校验。
